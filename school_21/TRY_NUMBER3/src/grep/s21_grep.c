...
void output(char **argv, struct options *opt) {
    ...
    if (fp != NULL) {
        while (fgets(opt->str, MAX, fp) != NULL) {
            ...
            switch (str) {
                case 0:
                    if (opt->c_flag && !opt->l && !opt->h && !opt->c) {
                    ...
                    if (opt->o && !opt->l && !opt->c) {
                        ...
                        while ((regexec(&regex, point, 1, &regmatch, 0) == 0)) {
                            printf("%.*s\n", (int)(regmatch.rm_eo - regmatch.rm_so), point + regmatch.rm_so);
        ...
        if (opt->l && c_count < 1 && opt->v) {
            printf("%s\n", argv[optind]);
        }

        if (opt->l && c_count > 0 && !opt->c) {
            printf("%s\n", argv[optind]);
        }

        if (opt->c && opt->c_flag && !opt->h) {
            printf("%s:", argv[optind]);
        }
